...
covdet \( = \) np.linalg.det \(\left( \operatorname{cov} + \mathrm{np}\text{. np. eye(dim)} * 0.001 \right)\) covinv \( = \) np.linalg.inv(cov + np.eye(dim) \( * 0.001\) ) xdiff \(f = \left( x {-} \text{mean} \right)\) .reshape \(\left( \left( 1,\dim \right) \right)\)
